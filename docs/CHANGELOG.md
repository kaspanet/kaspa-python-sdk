...
### Fixed
- `TransactionInput.to_dict()` now includes `computeBudget`. The field was already accepted by `from_dict` (defaulting to 0 when omitted) and readable on the object, but omitted on serialize, so v1 covenant submits that round-trip through dict/JSON used an effective budget of 0.
