...
### Added
- New `kaspa.experimental.silverscript` module — compile [SilverScript](https://github.com/kaspanet/silverscript) smart-contract source and build transaction scripts from Python. `silverscript.compile(source, constructor_args=[], *, allow_entrypoint_return=False, record_debug_infos=False)` returns a `CompiledContract` exposing `contract_name`, `compiler_version`, `without_selector`, `script` (locking-script bytes), `abi` (`FunctionAbiEntry`/`FunctionInputAbi`), and `state_layout`, plus `build_sig_script(function_name, args=[])` and `build_sig_script_for_covenant_decl(function_name, args=[], *, is_leader=False)` for unlocking scripts. Arguments are native Python values (`int`/`bool`/`str`/`bytes`/`list`/`dict`). Compilation errors raise `SilverScriptError`. Ships with `.pyi` type stubs for editor autocomplete and type checking. Shipped as a separate extension module (it links a different rusty-kaspa revision) bundled in the same `kaspa` wheel. It lives under the `kaspa.experimental` namespace because both SilverScript and these bindings are under active development and prone to breaking changes between releases (including the compiler's output) — pin your version and verify any contract end-to-end on a test network before locking real value.

### Fixed
- `requires-python` upper bound changed from `<=3.14` to `<3.15`. Under PEP 440 version ordering `<=3.14` excludes every 3.14 patch release (`3.14.1` and later), so `pip install kaspa` failed on real Python 3.14 builds (e.g. 3.14.6) with `requires a different Python`. Python 3.10–3.14 are all supported as before; only the metadata gate was wrong.

### Development
- The crate is now a Cargo workspace with three members: the core `kaspa` crate, `crates/silverscript` (the SilverScript bindings — a second `cdylib` linking a different rusty-kaspa revision), and `crates/core` (`kaspa-python-sdk-core`, a dependency-light crate sharing the `create_py_exception!` macro between the two). The SilverScript bindings are injected into the wheel under `kaspa/experimental/silverscript/`. `SilverScriptError` is defined with the same `#[pyclass(extends = PyException)]` approach as the core's wallet exceptions (so pyo3-stub-gen captures it automatically — no hand-written stub). Because that subclassing requires the full CPython API, the SilverScript module is **not** `abi3`: it's built once per CPython version, and CI (`release.yml`/`ci.yml`) builds + injects the matching `.so` for each interpreter before maturin builds that version's wheel.
