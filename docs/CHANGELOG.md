...
- Function `debug_call()` added to `kaspa.experimental.silverscript`, with result classes `DebugCallResult`, `FailureReport`, `FailureFrame`, and `DebugVariable`. Simulates a contract entrypoint call locally through SilverScript's source-level debug engine (the engine behind the upstream CLI debugger) and runs it to completion — no stepping or breakpoints. With `trace=True` the result additionally carries a per-statement execution trace (`TraceStep`: source line, statement text, enclosing function, and the variables in scope when the statement was reached), covering statements up to and including a failing one. Explicit `state` dicts in the `tx` scenario are validated against the contract's declared fields — unknown, missing, or mistyped fields raise `SilverScriptError`, even alongside a raw `utxo_script`/`script` override — and byte-array state fields accept `bytes`, int lists, and hex strings interchangeably. Only outputs carrying a covenant binding count toward the synthesized covenant `State` argument, so Auth-transition scenarios with plain change outputs behave as they would on-chain.
