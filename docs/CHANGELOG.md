...
### Changed
- **`kaspa.experimental.silverscript` (experimental): renamed to match SilverScript v1.0 vocabulary.** `CompiledContract.script` is now `CompiledContract.bytecode`; `FunctionAbiEntry` / `FunctionInputAbi` are now `EntryAbi` / `ParamAbi`; `EntryAbi.inputs` is now `EntryAbi.params`. SilverScript 1.0 renamed `script` to `bytecode` and reserves *function* for internal, non-entry functions, so the old names described a shape the compiler no longer has. `CompiledContract.state_layout` is unchanged and still `(start, len)` — it mirrors the compiler's `CompiledStateLayout`, which kept those names in 1.0. The `script`, `utxo_script`, and `signature_script` keys accepted by `debug_call(tx=...)` are unrelated: they are raw script bytes, matching `ScriptPublicKey.script`, and are unchanged.
