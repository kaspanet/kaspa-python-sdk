...
### Added
- New `kaspa.experimental.silverscript` module — compile [SilverScript](https://github.com/kaspanet/silverscript) (v1.0.0) contract source from Python. `compile()` returns a `CompiledContract` exposing the locking script (`bytecode`), ABI, state layout, and `template_hash`, plus `build_sig_script()` / `build_sig_script_for_covenant_decl()` for unlocking scripts. Each `EntryAbi` in the ABI carries the entrypoint's `name`, `params`, and four-byte `dispatch_tag`. `artifact_json()` returns the contract's portable ABI artifact as JSON — for the same source and constructor arguments this is byte-for-byte what the upstream `silverc` compiler writes, so artifacts are interchangeable between the two. `load_artifact()` reads that JSON back into a `ContractArtifact` — the same `bytecode`, `template_hash`, `state_span`, `abi` and `build_sig_script()` / `build_sig_script_for_covenant_decl()`, with no source and no compiler, so a contract can be compiled once in CI and its artifact shipped in place of its source. `ContractArtifact.check_consistency()` verifies an artifact's recorded template hash, state span and dispatch tags against its own bytecode, and `to_json()` serializes it back. On both classes `abi` is ordered alphabetically by entrypoint name, and `entry(name)` looks up a single entrypoint's `EntryAbi` directly, raising `SilverScriptError` for a name the contract doesn't declare.
...
- Class `ZkScriptBuilder` exposed to Python. Staged builder that turns RISC Zero zero-knowledge proofs into Kaspa transaction scripts, mirroring the WASM SDK: `new_r0()`, `commit_to_groth16()` / `commit_to_succinct()`, and `finalize_with_groth16_proof()` / `finalize_with_succinct_proof()`, plus fragment methods for composing scripts by hand (`add_data`, `push_r0_groth16_proof`, `push_r0_succinct_witness`, and `append_r0_groth16_verifier` / `append_r0_succinct_verifier` with their `*_with_fixed_journal` variants). `append_r0_groth16_verifier_dynamic_image_id()` is the same r0-over-groth16 verifier with the image id taken from the stack (`[..., journal_hash, compressed_proof, image_id]`) instead of baked into the script, so `append_r0_groth16_verifier(image_id)` is exactly `add_data(image_id)` followed by it; mirrors the WASM SDK's `appendR0Groth16VerifierDynamicImageId`. The produced P2SH scripts lock a UTXO behind on-chain verification of a specific proof (by image id, and optionally a fixed journal) via the `OpZkPrecompile` opcode. `new_r0()` also accepts the deprecated, ignored `covenants_enabled` argument described under Changed.
...
- Function `debug_call()` added to `kaspa.experimental.silverscript`, with result classes `DebugCallResult`, `FailureReport`, `FailureFrame`, `DebugVariable`, and `TraceStep`. Simulates a contract entrypoint call locally through SilverScript's source-level debug engine (the engine behind the upstream CLI debugger) and runs it to completion — no stepping or breakpoints. Omitting `function_name` calls the contract's first entrypoint in source order — not `abi[0]`, which is ordered by name. The `tx` argument describes the transaction the call is simulated in (inputs, outputs, covenant bindings and states), so covenant transitions and `tx` introspection can be debugged without a node. With `trace=True` the result additionally carries a per-statement execution trace (`TraceStep`: source line, statement text, enclosing function, and the variables in scope when the statement was reached). One statement is never traced: a covenant transition's `return(State { ... })`, whose produced state the engine verifies as a whole rather than stepping — so a transition body that is only that return traces to nothing, while its other statements trace normally.

### Changed
- **`kaspa.experimental.silverscript` (experimental): renamed to match SilverScript v1.0 vocabulary.** `CompiledContract.script` is now `CompiledContract.bytecode`; `FunctionAbiEntry` / `FunctionInputAbi` are now `EntryAbi` / `ParamAbi`; `EntryAbi.inputs` is now `EntryAbi.params`. SilverScript 1.0 renamed `script` to `bytecode` and reserves *function* for internal, non-entry functions, so the old names described a shape the compiler no longer has. `CompiledContract.state_layout` is now `CompiledContract.state_span`, still a `(offset, len)` pair — it names the same two numbers as the portable artifact's `compiled.state_span` and as `ContractArtifact.state_span`, so one spelling covers the attribute and the JSON key alike. The `script`, `utxo_script`, and `signature_script` keys accepted by `debug_call(tx=...)` are unrelated: they are raw script bytes, matching `ScriptPublicKey.script`, and are unchanged.
- **Affects `2.0.2rc1` only.** `kaspa.experimental.silverscript` first appeared in the `2.0.2rc1` pre-release, which also exposed `CompiledContract.without_selector` (removed — SilverScript 1.0 gives every entrypoint an unconditional dispatch tag) and produced different `template_hash` values (BLAKE2b-256 to BLAKE3) and different signature-script bytes. **A contract deployed against `2.0.2rc1` therefore has a different P2SH address under this release, and a UTXO locked by an rc1 script cannot be spent with a signature script built here.** The module is experimental; its API may change in breaking ways between releases.
- The `covenants_enabled` argument of `ScriptBuilder()` and `ScriptBuilder.from_script()` is deprecated and ignored, and the `ScriptBuilder.covenants_enabled` property is deprecated and always `True`. rusty-kaspa removed the flag along with the pre-Toccata script limits now that Toccata is active on every network, so every builder gets the covenant opcodes, explicit push opcodes, and the post-Toccata limits (1,000,000-byte script and element size, 1,000,000 ops) — a default `ScriptBuilder()` now accepts pushes over 520 bytes. Passing the argument (either value) or reading the property emits a `DeprecationWarning`; both will be removed in a future major version. Mirrors the WASM SDK, where `ScriptBuilderFlags.covenantsEnabled` is now a deprecated no-op.

### Fixed
- `UtxoEntry.to_dict()` and `UtxoEntryReference.to_dict()` now emit `covenantId` as a hex string instead of a `Hash` object, so dicts containing covenant UTXOs (including `Transaction.to_dict()` with such inputs) are JSON-serializable. `from_dict` already accepted either form.
- `TransactionInput.to_dict()` now includes `computeBudget`, including when nested under `Transaction.to_dict()`. The field was already accepted by `from_dict` (defaulting to 0 when omitted) and readable on the object, but omitted on serialize, so a v1 transaction that went through `to_dict()`, JSON, and `from_dict()` came back with an effective budget of 0.
...
- `SighashType.AllAnyOneCanPay` now maps to the `All | AnyOneCanPay` hashtype (`0x81`). Previously it mapped to the bare `AnyOneCanPay` bit (`0x80`), which is not a valid hashtype, so `create_input_signature` and `sign_input` with that type produced a digest and hashtype byte the script engine rejects. `NoneAnyOneCanPay` (`0x82`) and `SingleAnyOneCanPay` (`0x84`) were already correct. Upstream fix, rusty-kaspa `3563f06ba`.
...
- `Address(...)` and `Address.validate(...)` no longer panic on an address string whose checksum is valid but whose payload is the wrong length for its version, or that has no payload at all. Previously both surfaced a `pyo3_runtime.PanicException` — a `BaseException`, so `except Exception` did not catch it and `validate` raised instead of returning. `Address(...)` now raises a regular `Exception` and `validate` returns `False`. Upstream fix, rusty-kaspa #1138.

### Development
*Internal SDK changes — not visible through the Python package API.*

- The crate is now a Cargo workspace with three members: the core `kaspa` crate, `crates/silverscript` (the SilverScript bindings — a second `cdylib` with its own isolated rusty-kaspa dependency graph), and `crates/core` (`kaspa-python-sdk-core`, a dependency-light crate sharing the `create_py_exception!` macro between the two). The SilverScript bindings are injected into the wheel under `kaspa/experimental/silverscript/`.
- Bumped the pinned `rusty-kaspa` dependency for the core `kaspa` module from `cfafeb4c0` (v2.0.1) to `c338d49` (master). The range adds the `kaspa-txscript-zk-sdk` crate backing the new ZK bindings — pulling in `risc0-zkvm` and `borsh` as direct dependencies for decoding RISC Zero receipts — along with Windows link fixes for the ZK precompiles (kaspa-txscript enables risc0-zkvm-platform `export-syscalls` on Windows, resolving LNK2019). It also carries the Toccata cleanup series (upstream #1082-#1089, #1101), which drops the fork gates now that Toccata is active on every network: `EngineFlags::covenants_enabled`, the pre-Toccata script limits, the forked block version, and the pre-Toccata sigop counter are all gone. The Python-visible consequences are recorded under Changed (the `covenants_enabled` deprecation) and Fixed (the `SighashType` mapping); the remainder is consensus, mempool, and P2P internals with no Python-visible effect — mass and fee parameters (`mass_per_tx_byte`, `mass_per_script_pub_key_byte`, `mass_per_sig_op`, `storage_mass_parameter`) and opcode script-unit pricing are unchanged, and no RPC types changed.
- Bumped the pinned `rusty-kaspa` dependency for the core `kaspa` module to `01b532e8` (v2.1.0).
- The SilverScript member pins `silverscript-lang`, `silverscript-abi`, and `debugger-session` at `3ed9733` (SilverScript `v1.0.0`).
