...
- Method `ZkScriptBuilder.append_r0_groth16_verifier_dynamic_image_id()` — the r0-over-groth16 verifier fragment that takes the image id from the stack (`[..., journal_hash, compressed_proof, image_id]`) instead of baking it into the script. `append_r0_groth16_verifier(image_id)` is now exactly `add_data(image_id)` followed by it. Mirrors the WASM SDK's `appendR0Groth16VerifierDynamicImageId`.
...
### Changed
- The `covenants_enabled` parameter of `ScriptBuilder()`, `ScriptBuilder.from_script()`, and `ZkScriptBuilder.new_r0()` is deprecated and ignored, and the `ScriptBuilder.covenants_enabled` property is deprecated and always `True`. rusty-kaspa removed the flag along with the pre-Toccata script limits now that Toccata is active on every network, so every builder gets the covenant opcodes, explicit push opcodes, and the post-Toccata limits (1,000,000-byte script and element size, 1,000,000 ops). A default `ScriptBuilder()` now accepts pushes over 520 bytes, and `ZkScriptBuilder.new_r0(covenants_enabled=False)` no longer fails to finalize. Passing the parameter (either value) or reading the property emits a `DeprecationWarning`; both will be removed in a future major version. Mirrors the WASM SDK, where `ScriptBuilderFlags.covenantsEnabled` is now a deprecated no-op.
...
- `SighashType.AllAnyOneCanPay` now maps to the `All | AnyOneCanPay` hashtype (`0x81`). Previously it mapped to the bare `AnyOneCanPay` bit (`0x80`), which is not a valid hashtype, so `compute_sighash`, `create_input_signature`, and `sign_input` with that type produced a digest and hashtype byte the script engine rejects. `NoneAnyOneCanPay` (`0x82`) and `SingleAnyOneCanPay` (`0x84`) were already correct. Upstream fix, rusty-kaspa `3563f06ba`.
...
- Bumped the pinned `rusty-kaspa` dependency for the core `kaspa` module from `cfafeb4c0` to `78257f2` (master), which adds the `kaspa-txscript-zk-sdk` crate backing the new ZK bindings. Adds `risc0-zkvm` and `borsh` as direct dependencies for decoding RISC Zero receipts. The six-commit range also includes a docs update, Windows link fixes for the ZK precompiles (kaspa-txscript enables risc0-zkvm-platform `export-syscalls` on Windows, resolving LNK2019), an upstream RPC conversion fix (the `covenant` field on transaction outputs converted via `TryFrom<RpcOptionalTransactionOutput>`; the SDK never invokes that path), and an upstream fix to `XPrv`'s constant-time equality (not exposed in Python) — none with Python-visible effect.
- Bumped the pinned `rusty-kaspa` dependency for the core `kaspa` module from `78257f2` to `c338d49` (master).
