...
- Function `debug_call()` added to `kaspa.experimental.silverscript`, with result classes `DebugCallResult`, `FailureReport`, `FailureFrame`, and `DebugVariable`. Simulates a contract entrypoint call locally through SilverScript's source-level debug engine (the engine behind the upstream CLI debugger) and runs it to completion — no stepping or breakpoints. On failure the result carries a source-level report: the failing statement with source context, the inlined call stack, and every variable in scope per frame decoded to native Python values (locals, arguments, contract state, constructor arguments). `console.log` output is captured either way. An optional `tx` scenario dict controls the synthetic transaction (inputs/outputs, covenant ids, and previous/expected covenant state), enabling local dry runs of covenant state transitions before going on-chain.
