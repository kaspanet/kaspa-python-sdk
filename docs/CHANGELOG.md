...
- The SilverScript member pins `silverscript-lang`, `silverscript-abi`, and `debugger-session` at `3ed9733` (SilverScript `v1.0.0`), which tracks rusty-kaspa `a41a333b` — an ancestor of the core module's `c338d49`, so the two members resolve isolated dependency graphs. SilverScript 1.0 raises the Rust MSRV to 1.94.0.
