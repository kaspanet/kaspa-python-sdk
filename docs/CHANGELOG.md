...
### Development
- The crate is now a Cargo workspace; the SilverScript bindings live in `crates/kaspa-silverscript` (a second `cdylib` member) and are injected into the wheel under `kaspa/silverscript/`. The SilverScript module is built as a single `abi3` extension (one `silverscript.abi3.so` for all supported Python versions); CI (`release.yml`/`ci.yml`) builds and injects it on each runner before maturin builds the core wheels.
