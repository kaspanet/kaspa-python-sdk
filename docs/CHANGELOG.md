...
- Function `debug_call()` added to `kaspa.experimental.silverscript`, with result classes `DebugCallResult`, `FailureReport`, `FailureFrame`, and `DebugVariable`. Simulates a contract entrypoint call locally through SilverScript's source-level debug engine (the engine behind the upstream CLI debugger) and runs it to completion — no stepping or breakpoints. With `trace=True` the result additionally carries a per-statement execution trace (`TraceStep`: source line, statement text, enclosing function, and the variables in scope when the statement was reached).
...
- The SilverScript member pins `silverscript-lang`, `silverscript-abi`, and `debugger-session` at `3ed9733` (SilverScript `v1.0.0`), which tracks rusty-kaspa `a41a333b` — an ancestor of the core module's `c338d49`, so the two members still resolve two isolated dependency graphs / extension modules. SilverScript 1.0 raises the Rust MSRV to 1.94.0. The bindings were ported onto its portable ABI artifacts: `CompiledContract::build_sig_script*` and `CompiledContract.abi` were removed upstream, so `compile()` now also builds a `SilAbiArtifact` and encodes signature scripts through `silverscript_abi::encode_contract_entry_sig_script` / `encode_contract_covenant_decl_sig_script`, and the Python-visible `abi` is rebuilt from that artifact (still in source order, since the artifact keys entries alphabetically). Constructor arguments are lowered against the contract's declared parameter types via `artifact_value_to_expr` rather than as untyped literal expressions. Call and constructor arguments are also narrowed against those declared types before encoding — the portable ABI codec is variant-strict, so a `byte` position needs an `ArtifactValue::Byte`, which no Python value maps to on its own. The narrowing is type-directed, as it is in upstream's CLI (`debugger/session/src/args.rs`): a `byte` parameter accepts an `int` in `0..=255` (or a one-byte `bytes`) and anything outside that range raises `SilverScriptError`, while an `int` parameter is unaffected whatever its value. `build_sig_script_for_covenant_decl()` also checks the declaration name on the cov-bound follower path, which the codec resolves straight to the shared delegate entrypoint without consulting it: an unknown name raises `SilverScriptError` instead of building a well-formed script for the wrong call.
- SilverScript 1.0 changes the compiled output for every contract, so scripts built by these bindings differ from those built against the previously pinned `2a3961ca`: the template hash moved from BLAKE2b-256 to BLAKE3 (both the Rust `template_hash()` and the `templateHash()` builtin's emitted opcode, so they still agree), and entrypoint dispatch moved from a positional integer selector to a four-byte BLAKE3 dispatch tag — every entry now carries one, so the `CompiledContract.without_selector` property was removed. **A contract's P2SH address therefore changes**, and a UTXO locked by a script from the old revision cannot be spent with a signature script built by the new one. This only affects contracts already deployed against a pre-release build of this module; the `examples/silverscript/` Counter example starts from genesis on each run and is unaffected.
