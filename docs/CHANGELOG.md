...
- Function `debug_call()` added to `kaspa.experimental.silverscript`, with result classes `DebugCallResult`, `FailureReport`, `FailureFrame`, `DebugVariable`, and `TraceStep`. Simulates a contract entrypoint call locally through SilverScript's source-level debug engine (the engine behind the upstream CLI debugger) and runs it to completion — no stepping or breakpoints. The `tx` argument describes the transaction the call is simulated in (inputs, outputs, covenant bindings and states), so covenant transitions and `tx` introspection can be debugged without a node. With `trace=True` the result additionally carries a per-statement execution trace (`TraceStep`: source line, statement text, enclosing function, and the variables in scope when the statement was reached).
