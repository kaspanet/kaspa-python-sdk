...
| `mkdocstrings` | Renders docstrings from `kaspa/` stub files into HTML |
...
4. Outputs `kaspa/__init__.pyi` and `kaspa/exceptions/__init__.pyi`

### API Reference Generation

At docs build time (`mkdocs build` or `mkdocs serve`):

1. `docs/gen_ref_pages.py` parses `kaspa/__init__.pyi` and `kaspa/exceptions/__init__.pyi` for classes, functions, enums, and exceptions
