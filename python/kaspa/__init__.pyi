...
class AccountDescriptor:
    ...
    def balance(self) -> typing.Optional[Balance]:
        ...
        """
    @property
    def prv_key_data_ids(self) -> typing.Optional[builtins.list[PrvKeyDataId]]:
        r"""
        The associated prv key data ids
        """
    @property
    def receive_address(self) -> typing.Optional[Address]:
        r"""
        The current receive address as a string, or None if not available.
        """
    @property
    def change_address(self) -> typing.Optional[Address]:
        r"""
        The current change address as a string, or None if not available.
        """
    @property
    def account_index(self) -> typing.Optional[builtins.int]:
        r"""
        The BIP44 account index. None for account kinds that are not derived from
        an account index (e.g. keypair accounts).
        """
    @property
    def xpub_keys(self) -> typing.Optional[builtins.list[builtins.str]]:
        r"""
        ...
        """
    @property
    def ecdsa(self) -> typing.Optional[builtins.bool]:
        r"""
        Whether the account uses ECDSA signatures (vs Schnorr). None if not applicable.
        """
    @property
    def receive_address_index(self) -> typing.Optional[builtins.int]:
        r"""
        ...
        """
    @property
    def change_address_index(self) -> typing.Optional[builtins.int]:
        r"""
        ...
        index. None for account kinds without HD derivation.
        """
    def get_addresses(self) -> typing.Optional[builtins.list[Address]]:
        r"""
...
# =============================================================================
# Notification Bodies (the inner notification struct delivered as event["data"])
# =============================================================================

class RpcBlockAddedNotification(TypedDict):
    """Body for the `block-added` notification.

    Delivered as `event["data"]` to listeners registered with
    `add_event_listener("block-added", ...)`.
    """
    block: RpcBlock


class RpcVirtualChainChangedNotification(TypedDict):
    """Body for the `virtual-chain-changed` notification.

    `acceptedTransactionIds` is only populated when the subscription was
    opened with `include_accepted_transaction_ids=True`.
    """
    removedChainBlockHashes: list[str]
    addedChainBlockHashes: list[str]
    acceptedTransactionIds: list[RpcAcceptedTransactionIds]
...
class RpcFinalityConflictNotification(TypedDict):
    """Body for the `finality-conflict` notification."""
    violatingBlockHash: str


class RpcFinalityConflictResolvedNotification(TypedDict):
    """Body for the `finality-conflict-resolved` notification."""
    finalityBlockHash: str


class RpcSinkBlueScoreChangedNotification(TypedDict):
    """Body for the `sink-blue-score-changed` notification."""
    sinkBlueScore: int


class RpcVirtualDaaScoreChangedNotification(TypedDict):
    """Body for the `virtual-daa-score-changed` notification."""
    virtualDaaScore: int


class RpcPruningPointUtxoSetOverrideNotification(TypedDict, total=False):
    """Body for the `pruning-point-utxo-set-override` notification.

    The body has no fields; the notification fires as a signal only.
    """
    pass


class RpcNewBlockTemplateNotification(TypedDict, total=False):
    """Body for the `new-block-template` notification.
...
# =============================================================================
# Event Payloads (the dict an `add_event_listener` callback receives)
# =============================================================================

class BlockAddedEvent(TypedDict):
    """Payload delivered for the `block-added` event."""
    type: typing.Literal["block-added"]
    data: RpcBlockAddedNotification


class VirtualChainChangedEvent(TypedDict):
    """Payload delivered for the `virtual-chain-changed` event."""
    type: typing.Literal["virtual-chain-changed"]
    data: RpcVirtualChainChangedNotification


class FinalityConflictEvent(TypedDict):
    """Payload delivered for the `finality-conflict` event."""
    type: typing.Literal["finality-conflict"]
    data: RpcFinalityConflictNotification


class FinalityConflictResolvedEvent(TypedDict):
    """Payload delivered for the `finality-conflict-resolved` event."""
    type: typing.Literal["finality-conflict-resolved"]
    data: RpcFinalityConflictResolvedNotification


class SinkBlueScoreChangedEvent(TypedDict):
    """Payload delivered for the `sink-blue-score-changed` event."""
    type: typing.Literal["sink-blue-score-changed"]
    data: RpcSinkBlueScoreChangedNotification


class VirtualDaaScoreChangedEvent(TypedDict):
    """Payload delivered for the `virtual-daa-score-changed` event."""
    type: typing.Literal["virtual-daa-score-changed"]
    data: RpcVirtualDaaScoreChangedNotification


class PruningPointUtxoSetOverrideEvent(TypedDict):
    """Payload delivered for the `pruning-point-utxo-set-override` event."""
    type: typing.Literal["pruning-point-utxo-set-override"]
    data: RpcPruningPointUtxoSetOverrideNotification


class NewBlockTemplateEvent(TypedDict):
    """Payload delivered for the `new-block-template` event."""
    type: typing.Literal["new-block-template"]
    data: RpcNewBlockTemplateNotification


class UtxosChangedEvent(TypedDict):
    """Payload delivered for the `utxos-changed` event.

    Note: unlike most notifications, the body is flattened onto the
    event dict rather than nested under a `data` key.
    """
    type: typing.Literal["utxos-changed"]
    added: list[RpcUtxosByAddressesEntry]
    removed: list[RpcUtxosByAddressesEntry]


class ConnectEvent(TypedDict):
    """Payload delivered for the `connect` RPC control event.

    Fires when the underlying WebSocket connects (including reconnects).
    `add_event_listener("connect", ...)` alone is enough — no
    `subscribe_*` is required.
    """
    type: typing.Literal["connect"]
    rpc: str | None


class DisconnectEvent(TypedDict):
    """Payload delivered for the `disconnect` RPC control event.

    Fires when the underlying WebSocket drops.
    `add_event_listener("disconnect", ...)` alone is enough — no
    `subscribe_*` is required.
    """
    type: typing.Literal["disconnect"]
    rpc: str | None


NotificationEventPayload = typing.Union[
    BlockAddedEvent,
...
"""Discriminated union of every dict shape that an event listener can
receive. Narrow on the `type` field to access event-specific keys."""